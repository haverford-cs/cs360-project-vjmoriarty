"""
Fine tuning functions for LSTM
Authors: Vincent Yu
Date: 12/13/2020
"""
...
# Local imports
from data.dataset import generate_dset_LSTM
from data.info import population_by_state
from models.conv_lstm import run_lstm
from models.settings import random, random_size_LSTM, params_LSTM, epochs, \
    rescale
...
def main():
    ...
    # Fine tune LSTM
    cases_log_LSTM, deaths_log_LSTM = fine_tune_LSTM(
        random=random,
        random_size=random_size_LSTM
    )

    # Take the five best params for cases and deaths predictions
    ft_params_lstm = {
        'cases': sorted(cases_log_LSTM.items(), key=lambda x: x[1])[:5],
        'deaths': sorted(deaths_log_LSTM.items(), key=lambda x: x[1])[:5],
    }
